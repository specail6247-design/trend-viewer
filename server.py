...
# 클라우드 호스팅(Render 등)은 PORT 환경변수로 포트를 지정합니다. 로컬은 8778 그대로.
...
# syndication 타임라인이 429로 막혔을 때 쓰는 예비 경로(니터 미러 → 트윗 ID → 임베드 API)
NITTER_HOSTS = ["nitter.net", "xcancel.com", "nitter.poast.org"]


def _x_embed_token(tweet_id: str) -> str:
    """cdn.syndication.twimg.com/tweet-result 호출용 토큰(트위터 임베드 위젯과 같은 계산)."""
    x = int(tweet_id) / 1e15 * math.pi
    digits = "0123456789abcdefghijklmnopqrstuvwxyz"
    intpart, frac = int(x), x - int(x)
    s = ""
    while intpart:
...
def _x_tweet_detail(tweet_id: str, username: str):
    url = ("https://cdn.syndication.twimg.com/tweet-result?id=%s&token=%s"
           % (tweet_id, _x_embed_token(tweet_id)))
    try:
        t = http_json(url, timeout=10)
    except Exception:
        return None
    ...
    return {
        ...
        "replies": t.get("conversation_count") or 0,
        "retweets": 0,
        "views": 0,
        "media": media,
        "url": "https://x.com/%s/status/%s" % (username, t.get("id_str", tweet_id)),
        "createdAt": t.get("created_at", ""),
    }


def _x_posts_via_nitter(username: str, limit=12):
    """니터 RSS로 최근 트윗 ID를 얻고 임베드 API로 좋아요·댓글 수를 채웁니다.
    임베드 API에는 리트윗·조회수가 없어 두 값은 0으로 남습니다."""
    ids = []
    for host in NITTER_HOSTS:
        try:
            _, body = http_get("https://%s/%s/rss" % (host, quote(username)), timeout=12)
            ids = list(dict.fromkeys(re.findall(r"/status/(\d+)", body.decode("utf-8", "ignore"))))
            if ids:
                break
        except Exception:
            continue
...
def fetch_x_posts(username: str):
    ...
    try:
        ...
        if not m:
            return _x_posts_via_nitter(username)
    ...
    return posts or _x_posts_via_nitter(username)
...
# 검색엔진 크롤러에게는 메타가 게시물을 서버렌더링(HTML 내 JSON)으로 내려줍니다.
THREADS_CRAWLER_UA = "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)"


def _threads_posts_via_ssr(username: str):
    """프로필 HTML(크롤러 UA)에 포함된 게시물 JSON을 파싱합니다.
    doc_id가 필요 없어 GraphQL 조회가 막혀 있어도 동작합니다."""
    try:
        _, body = http_get("https://www.threads.com/@" + quote(username),
                           headers={"User-Agent": THREADS_CRAWLER_UA}, timeout=15)
    except Exception:
        return []
    html = body.decode("utf-8", "ignore")
    posts = []
    for m in re.finditer(r'<script type="application/json"[^>]*data-sjs[^>]*>(.*?)</script>', html, re.S):
        s = m.group(1)
        if "thread_items" not in s:
            continue
        try:
            data = json.loads(s)
        except Exception:
            continue
        posts.extend(_parse_threads(data, username))
    seen, unique = set(), []
    for p in posts:
...
def fetch_threads_posts(username: str):
    posts = _threads_posts_via_ssr(username)
    if posts:
        return posts
...
def lan_ip():
    """같은 와이파이의 다른 기기(폰·태블릿)가 접속할 때 쓸 이 컴퓨터의 IP를 찾습니다."""
    import socket
    try:
...
if __name__ == "__main__":
    ...
    ip = lan_ip()
    if ip:
        print(f"  📱 폰·아이패드(같은 와이파이): http://{ip}:{PORT}")
